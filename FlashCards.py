...
class Flash:  
    ...
    def update(self):  # run through the terms_list, find the one that relates to the dictionary,
        u = 1
        new_list = {}
        for j in self.__all_cards:
            new_list.update({f"Card {u}": self.__all_cards[j]})
            u += 1
        self.__all_cards = new_list
    ...
    def delete_card(self):
        ...
        j = -1
        while check and (
                sure != "nvm"):  # when the name is found in the list, check should be false and the loop should stop
            delete = input("Please type the term of the card you want to delete: ")
            for i in range(len(self.__terms_list)):
                if self.__terms_list[i] == delete:
                    ...
                    self.__counter -= 1
            if (j >= 0):
                self.__terms_list.pop(j)
                self.__definitions_list.pop(j)
                self.update()
            if (check):
    ...
    def quiz(self):
        ...
        while again == "yes":
            ...
            print(
                "You'll see a definition and try to type the correct term that matches it. Make sure the term you type is exact.\n")
            points = 0
            ...
            print(
                f"Your quiz grade is {points / len(self.__terms_list) * 100} percent.")  # optional: saying good job, poor job, try again, or a letter grade
